...
void PAIReDONNXJetTagsProducer::make_inputs(btagbtvdeep::PAIReDFeatures features) {
  ...
  // c_pf candidates for jet 2
  for (std::size_t c_pf_n = 0; c_pf_n < max_c_pf_n; c_pf_n++) {
    const auto& c_pf_features_j2 = features.c_pf_features_j2.at(c_pf_n);
    ptr = &data_[kChargedCandidates_j2][offset + c_pf_n * n_features_cpf_];
    start = ptr;
    *ptr = c_pf_features_j2.btagPf_trackEtaRel;
    ...
    *(++ptr) = c_pf_features_j2.drminsv;
    *(++ptr) = c_pf_features_j2.vtx_ass;
    *(++ptr) = c_pf_features_j2.puppiw;
    *(++ptr) = c_pf_features_j2.chi2;
    *(++ptr) = c_pf_features_j2.quality;
    assert(start + n_features_cpf_ - 1 == ptr);
  }

  // n_pf candidates for jet 1
  ...
  for (std::size_t n_pf_n = 0; n_pf_n < max_n_pf_n; n_pf_n++) {
    ...
    *(++ptr) = n_pf_features_j1.isGamma;
    *(++ptr) = n_pf_features_j1.hadFrac;
    *(++ptr) = n_pf_features_j1.drminsv;
    *(++ptr) = n_pf_features_j1.puppiw;
    assert(start + n_features_npf_ - 1 == ptr);
  }

  // n_pf candidates for jet 2
  auto max_n_pf_n = std::min(features.n_pf_features_j2.size(), (std::size_t)n_npf_);
  for (std::size_t n_pf_n = 0; n_pf_n < max_n_pf_n; n_pf_n++) {
    const auto& n_pf_features_j2 = features.n_pf_features_j2.at(n_pf_n);
    ptr = &data_[kNeutralCandidates_j2][offset + n_pf_n * n_features_npf_];
    start = ptr;
    *ptr = n_pf_features_j2.ptrel;
    *(++ptr) = n_pf_features_j2.etarel;
    *(++ptr) = n_pf_features_j2.phirel;
    *(++ptr) = n_pf_features_j2.deltaR;
    *(++ptr) = n_pf_features_j2.isGamma;
    *(++ptr) = n_pf_features_j2.hadFrac;
    *(++ptr) = n_pf_features_j2.drminsv;
    *(++ptr) = n_pf_features_j2.puppiw;
    assert(start + n_features_npf_ - 1 == ptr);
  }
